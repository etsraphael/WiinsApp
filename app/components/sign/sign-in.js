        ...
        if (!newProps.MyUser.isLoading && newProps.MyUser.error) {
            switch (newProps.MyUser.error) {
                case 'email_or_password_invalid': {
                    ...
                    return this.setState({ error: I18n.t('ERROR-MESSAGE.Pseudo-Or-Email-Incorrect') })
                }
                case 'account_deactivated': {
                    // return Snackbar.show({ text: I18n.t('ERROR-MESSAGE.Account_desactivated'), duration: Snackbar.LENGTH_LONG })
                    return this.setState({ error: I18n.t('ERROR-MESSAGE.Account_desactivated') })
                }
            }
        }
    }
                ...
                <Sign label="Sign In" navigation={this.props.navigation}>
                    ...
                    >
                        <ScrollView showsVerticalScrollIndicator={false}  style={{ flex: 1 }} bounces>
                            <Text style={[styles.mainLargeText, { marginTop: 36 }]}>Welcome Back</Text>
                            ...
                            <View style={{ marginTop: 48 }}>
                                ...
                                <WGradientButton text="Sign in" style={styles.createButton} onPress={() => this._login()} />
                            </View>
                        </ScrollView>
                    </ErrorPresenter>
                </Sign>
